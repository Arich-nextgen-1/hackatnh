...
${JSON.stringify((rehabsData as any[]).map(r => ({ id: r.id, name: r.name, address: r.address, phone: r.phone || '', programs: r.programs ?? r.services ?? [], rating: r.rating, description: r.description ?? '' })), null, 2)}
`;
